...
const Game: React.FC<WalletAmount> = ({ wallet, setWallet }) => {
    ...
    const gameFinished = (): void => {
        ...
        if (bets.number.value === null) {
            setMsg(prev => ({ ...prev, number: `${randomNumber}` }))
        } else {
    ...
    return (
        <div className="w-full overflow-y-auto px-5">
            {(msg.color || msg.number) && (
                <div className="absolute left-1/2 top-1/2 transform -translate-1/2 w-full flex flex-col  bg-blue-500 gap-y-10 justify-center items-center z-1 p-5 font-black text-sm sm:text-lg rounded">
                    {msg.color && <p className="w-1/2 p-2 rounded-lg flex flex-col justify-center items-center text-center">
                        <span>{msg.color}</span>
                    </p>}
                    {msg.number && <p className="w-1/2 p-2 rounded-lg flex flex-col justify-center items-center text-center">
            ...
            {/* <div className="w-full h-full overflow-y-auto pb-20 scroll-hide"> */}
            {/* Color Choices */}
            <BetSelector
                type="color"
